...
function buildCard(post) {
  ...
  return `
    <article class="blog-card anim-reveal" onclick="openPost(${post.id}, '${KFM.slugify(post.title)}')">
      ...
      <div class="blog-card-body">
        ...
        <div class="card-meta">
          <span>${KFM.formatDate(post.created_at)} · ${readingTime(post.body)} min read</span>
...
// ── Scroll reveal for cards (dynamism) ────────────────────
function initScrollReveal() {
  const els = document.querySelectorAll('.anim-reveal:not(.revealed)');
...
// ── Open post ────────────────────────────────────────────
function openPost(id, slug) {
  window.location.href = 'post.html?id=' + id + (slug ? '&slug=' + slug : '');
}
...
async function loadPublicDesign() {
  try {
    ...
    if (!data) return;
    const vars = {};
    if (data.theme && PUB_THEMES[data.theme]) Object.assign(vars, PUB_THEMES[data.theme]);
    if (data.font && PUB_FONTS[data.font]) vars['--font-display'] = PUB_FONTS[data.font];
    if (data.font_size) vars['--font-size-base'] = data.font_size + 'px';
    Object.entries(vars).forEach(([k,v]) => document.documentElement.style.setProperty(k,v));
    try { localStorage.setItem('kfm_theme_cache', JSON.stringify(vars)); } catch(e) {}
